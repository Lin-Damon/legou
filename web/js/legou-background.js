...
LGB.queryCount = function(keyword,successFN,errorFN){
	var Obj = Bmob.Object.extend("Goods");
	var query1 = new Bmob.Query(Obj);
	query1.equalTo("type",keyword);
	var query2 = new Bmob.Query(Obj);
	query2.equalTo("subtype",keyword);
	var mainQuery = Bmob.Query.or(query1,query2);
	mainQuery.count({
...
LGB.getShippingAddress = function (successFN,errorFN) {
	...
	query.find({
		success:function (obj){
			console.log(obj);
			if(obj[0]){
				var res= obj[0].attributes.address||[];
				successFN(res);
			}else{
				successFN([]);
			}
