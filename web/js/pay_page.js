...
//	window.location.href = "pay.html";
	...
	LGB.getShippingAddress(function(address){
		userAddress = address || [];
		var selectAddress = userAddress[index];
		console.log(selectAddress);
		LGB.getUserOrder(function(obj){
			var orders = obj;
			console.log(orders);
			for(var i in orders){
				var user_info = orders[i].attributes.userinfo;
				var json_userinfo = JSON.parse(user_info);
				console.log(typeof(json_userinfo));
				json_userinfo.useraddress = userAddress[index];
				
				LGB.updateOrder(obj[i],{"userinfo":JSON.stringify(json_userinfo)},function(obj){
...
function chooseAdress(){
	var about_addresses = document.getElementsByClassName("address_div");
	var about_address = this;
	for(var i=0;i<about_addresses.length;i++){
		about_addresses[i].style='';
	}
	this.style.backgroundColor='orange';
	
	index = getIndex(about_address,about_addresses);
	console.log(index);	
...
//	},function(error){console.log(error);})
	...
	LGB.getShippingAddress(function(address){
		userAddress = address || [];
		userAddress.push(address_date);
		
		LGB.updateShippingAddress({"address":userAddress},function(obj){
			console.log("上传地址成功");
			window.location.reload();
		},function(error){
			console.log(error);
...
function dataFN(data) {
	...
	address_div.addEventListener("click",chooseAdress,false);
