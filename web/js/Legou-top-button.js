...
LeguTop.prototype.checkUser = function (){
	...
	if (user) {
		...
		document.getElementsByClassName("nav2")[0].addEventListener("click",function(){
			window.open("./userOrder.html","_self");
		})
	}
	return user||null;
}
//判断用户登录，没登录跳转到登录页面
 LeguTop.prototype.judgeUser=function(){
	LGB.init();
	var user = LGB.getCurrentUser();
	if(!user){
	     alert("还没登录，请先登录");
